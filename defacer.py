...
class Defacer(object):
    ...
    def Deidentification_image_dcm(self, where, dicom_path, dest_path, verif_path, url, prefix, Model=model):
        ...
        try:
            ...
            # --- ADD THIS TO SAVE THE MASK (as NIfTI for easy viewing) ---
            mask_to_save = np.argmax(results, axis=-1).astype('int16')

            # Since DICOM doesn't have an affine like NIfTI, 
            # we save it as a simple NIfTI file in the output folder
            mask_filename = os.path.join(dest_path, "predicted_mask.nii.gz")
            nib.save(nib.Nifti1Image(mask_to_save, np.eye(4)), mask_filename)
            print(f"Mask saved successfully at: {mask_filename}")
            # -------------------------------------------------------------
    ...
    def Deidentification_image_nii(self, where, nfti_path, dest_path, verif_path, url, prefix, Model=model):
        ...
        try:
            ...
            mask_to_save = np.argmax(results, axis=-1).astype('int16')
            mask_filename = os.path.join(os.path.dirname(dest_path), "MASK_" + os.path.basename(nfti_path))
            nib.save(nib.Nifti1Image(mask_to_save, raw_img.affine, raw_img.header), mask_filename)
            print(f"Mask saved successfully at: {mask_filename}")
            # ---------------------------------
