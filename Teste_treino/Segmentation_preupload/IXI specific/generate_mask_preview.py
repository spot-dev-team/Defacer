...
# --- CONFIGURAÇÃO ---
BASE_DIR = r"D:\Tese_BDs\Defacer\extras"
...
def generate_qa_images():
    ...
    for i, subj in enumerate(subjects):
        ...
        mask_dir = os.path.join(subj_path, "training_masks") 
        if not os.path.exists(mask_dir): 
            print(f" -> [ERRO] training_masks não encontrado para {subj}. Saltando.")
            continue
        
        mask_path = os.path.join(mask_dir, "mask_4_classes.nii.gz")
        ...
        # --- ALTERAÇÃO AQUI ---
        # Se já existir imagem, apaga para garantir que é gerada uma nova
        if os.path.exists(qa_output):
            try:
                os.remove(qa_output)
                # print(f" -> Imagem antiga removida.") # Descomenta se quiseres ver no log
            except OSError as e:
                print(f" -> Aviso: Não foi possível apagar a imagem antiga: {e}")
