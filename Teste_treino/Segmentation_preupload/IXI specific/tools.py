...
class SpotSegmenter:
    ...
    def process(self):
        ...
        if np.any(self.masks['face_main']):
            self._find_nose_tip()
            self._extract_nose_tunnel() # Versão Híbrida BackWall + Smart Drop
    ...
    def _find_smart_nose_bottom(self):
        """
        Percorre o perfil do nariz para baixo até encontrar o ponto onde ele 
        recua ao máximo (Subnasale) antes do lábio começar a sair.
        """
        # Procura no máximo 25mm abaixo da ponta (segurança)
        max_search_mm = 25.0 
        max_search_vox = int(max_search_mm / self.voxel_sizes[2])
        
        start_z = self.nose_tip_coords[2]
        current_min_y = self.nose_tip_coords[1] # Começamos na ponta (Y máximo)
        best_z = start_z
        
        # Iterar Z "para baixo" (direção da boca)
        for i in range(1, max_search_vox):
            z_curr = int(start_z + (i * self.z_direction))
            
            # Verificar limites da imagem
            if z_curr < 0 or z_curr >= self.face_data.shape[2]: break
            
            # Obter a fatia da cara neste Z
            slice_mask = self.masks['face_main'][:, :, z_curr]
            if not np.any(slice_mask): continue 
            
            # Encontrar o ponto mais à frente (Max Y) nesta fatia
            # Limitamos a busca em X para não apanhar bochechas laterais, só o centro
            center_x = self.nose_tip_coords[0]
            margin_x = 10 # +/- 10 voxels à volta do centro
            x_min_s = max(0, center_x - margin_x)
            x_max_s = min(self.face_data.shape[0], center_x + margin_x)
            ...
            # O Y mais anterior nesta linha (lembra-te que coords[:,1] é o Y original relativo ao strip)
            # Precisamos do Y absoluto. O argwhere devolve indices relativos ao strip.
            # Mas espera, slice_mask é (X, Y). center_strip é (X_crop, Y).
            # coords[:, 1] é o Y real.
            ...
            if y_curr <= current_min_y:
                ...
                # O Y aumentou (lábio a sair).
                # Se já descemos pelo menos uns 5mm... Paramos. Achámos o fundo do nariz.
                dist_mm = i * self.voxel_sizes[2]
                if dist_mm > 5.0: 
                    return best_z
        
        # Se nunca encontrou o lábio (ex: barba grande ou geometria estranha), 
        # devolve o ponto onde o nariz estava mais "fundo" dentro do limite de 25mm
        return best_z

    def _extract_nose_tunnel(self):
        # LÓGICA HÍBRIDA: BackWall + Convex Hull + SMART DROP
        ...
        # Definição do Cilindro (Largura apenas)
        radius_width_mm = 35.0 
        rad_x = int(radius_width_mm / self.voxel_sizes[0])
        
        # --- AQUI ESTÁ A CORREÇÃO ---
        # Em vez de 35mm fixos, calculamos onde o nariz acaba
        z_smart_bottom = self._find_smart_nose_bottom()
        self.nose_z_bottom_limit = z_smart_bottom
        
        z_top = int(z_eyes)
        z_bottom = int(z_smart_bottom)
        
        z_min, z_max = min(z_top, z_bottom), max(z_top, z_bottom)
        
        # Ajuste do cilindro geométrico para usar a altura correta
        z_mid = (z_min + z_max) / 2
        z_radius = (z_max - z_min) / 2
        # Se z_radius for muito pequeno (nariz curto), damos um mínimo de segurança
        if z_radius < 1: z_radius = 1
    ...
    def _extract_mouth_relative(self):
        ...
        gap_voxels = int(2.0 / self.voxel_sizes[2]) # Reduzi para 2mm para não ficar buraco grande
        ...
        # Boca é tudo o que está abaixo do corte
    ...
    def save_final_mask(self):
        ...
        if self.masks['ears'] is not None: combined[self.masks['ears']==1] = 2 
