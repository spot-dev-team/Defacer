...
NEW_MODEL_NAME = 'modelo_final_augmented.h5'
BATCH_SIZE = 1 
EPOCHS = 60
LEARNING_RATE = 1e-4
...
class AdniDataGenerator(tf.keras.utils.Sequence):
    ...
    def remap_labels(self, mask):
        ...
        # O valor 1 (Lixo) não é copiado, logo fica a 0 (Fundo)
    ...
    def __data_generation(self, list_ids_temp):
        ...
        for i, ID in enumerate(list_ids_temp):
            ...
            try:
                ...
                # Apenas carrega mask_4_classes.nii.gz, pois é a que tem os 5 segmentos
                mask_path = os.path.join(subj_path, "training_masks", "mask_4_classes.nii.gz")
                
                if not os.path.exists(mask_path):
                     # Se não tiver a máscara correta, salta este exame para não estragar o treino
                     continue
            ...
            # 2. REMAPEAMENTO DE LABELS (A Correção Principal)
            msk = np.round(msk)
...
def train():
    ...
    # Divisão por Paciente (Segurança Contra Data Leakage)
    try:
    ...
    gen_train = AdniDataGenerator(train_ids, BASE_DIR, batch_size=BATCH_SIZE, augment=True)
    ...
    callbacks = [
        ...
        CSVLogger('training_log_final.csv', append=True),
